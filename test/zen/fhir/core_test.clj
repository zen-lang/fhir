...
(t/deftest ^:kaocha/pending different-extension-syntax
  ;; TODO: How should it be generated
  ;; The zen fhir uses differential of StructureDefinition.
  ;; The Base FHIR Extension definition allows value to have any time.
  ;; If there is no value[x] in differential we don't modify it
  ;; that is the extension value could be of any FHIR type
  ;; But value[x].valueCodeableConcept (or any other) could be redefined
  ;; This means that any type is allowed but if the type is CodeableConcept
  ;; then this type must conform the definition given for the type.

  (t/is false "Fail until decided")

  (matcho/match
   (sut/process-on-load {:fhirVersion "3.0.2"
                         :type "Extension"
                         :resourceType "StructureDefinition"
                         ...
                         :differential {:element
                                        [{:id "Extension.url"
                                          :path "Extension.url"
                                          :fixedUri "http//nictiz.nl/fhir/StructureDefinition/zib-AddressInformation-AddressType"}
   ...
   {:| {:CodeableConcept {:sliceName "valueCodeableConcept"
                          :type "CodeableConcept"}}})
