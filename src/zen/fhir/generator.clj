             ...
             (let [base (when-let [b (:baseDefinition res)] (last (str/split b  #"/")))]
               (cond-> 
                   (assoc tp
                          ...
                          :zen/desc (get-in res [:differential :element 0 :short])
...
(def res-prof (cheshire.core/parse-string (slurp (str path "fhir/profiles-resources.json")) keyword))

(def resources (->> (concat (:entry types) (:entry res-prof))
                    (mapv :resource)
                    (reduce (fn [acc x]
                              (assoc acc (:id x) (-> (dissoc x :snapshot :text :meta :publisher :fhirVersion :purpose :mapping :extension :version :date :derivation :description)
                                                     (update-in [:differential :element] (fn [ds]
                                                                             (->> ds
                                                                                  (mapv (fn [x]
                                                                                          (cond-> 
                                                                                           (dissoc x :definition :id :alias :mapping
                                                                                                   :requirements :comment :isModifierReason :meaningWhenMissing)
...
;;   (spit (str path "zrc/datatypes.edn") edn))

;; (def resources (cheshire.core/parse-string (slurp (str path "fhir/profiles-resources.json")) keyword))
